...
function setupServerHmr(
  ...
  async function runSubscription() {
    ...
    try {
      ...
      await clear()
      // Re-subscribe so server Fast Refresh continues working for the rest of
      // the dev session.
      runSubscription().catch((retryErr) => {
        console.error('[Server HMR] Resubscription failed:', retryErr)
      })
    }
  }

  // Start listening for changes in background
  runSubscription().catch(async (err) => {
